...
Your job is to generate one daily challenge combining a math/physics problem
with a programming implementation task.
...
- The programming task must implement or verify the math/physics result.
- Use only standard libraries (math, sys, stdio). No external packages.
...
# Difficulty rotates with the user's trail phase: early phases are beginner,
# middle intermediate, late advanced.
PHASE_DIFFICULTY = {
...
def _extract_json(text: str) -> dict:
    """Parse a JSON object out of the model response, tolerating stray fences."""
    text = text.strip()
    if text.startswith("```"):
        text = text.strip("`")
...
def _generate_via_claude(difficulty: Difficulty, today: date) -> dict:
    # Note: claude-fable-5 rejects sampling params (temperature/top_p/top_k),
    # so none are sent — the spec's temperature setting does not apply here.
    client = _client()
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Pick a category from: physics, math, algorithms."
                ),
            }
        ],
    )
...
def get_or_create_today_challenge(db: Session, user: User) -> DailyChallenge:
    ...
    try:
        data = _generate_via_claude(difficulty, today)
    except Exception:
    ...
    challenge = DailyChallenge(
        ...
        category=str(data.get("category", "math"))[:64],
